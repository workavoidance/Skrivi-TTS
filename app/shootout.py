...
import winsound
from core import KOKORO_VOICES, DATA, ROOT, VERSION, Library, catalog, defaults, validate, read_json, write_json, sha256
...
ENGLISH_TEXT = 'The rain had stopped by the time we reached the station. Sarah looked at the empty platform and smiled. We had missed the last train, but neither of us was in a hurry to go home.'
LABELS = {'voice': 'English voice', 'seed': 'Seed', 'steps': 'Diffusion steps (4–20)', 'guidance': 'Guidance (1–4)',
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.runs = {}
        self.fields = {}
        selected = next((m for m in self.models if m['id'] == self.saved.get('last_model_id')), self.models[0])
        self.model_name = tk.StringVar(value=selected['name'])
    ...
    def change_model(self):
        ...
        if self.text.get('1.0', 'end').strip() in (DEFAULT_TEXT, ENGLISH_TEXT):
            self.text.delete('1.0', 'end')
            self.text.insert('1.0', ENGLISH_TEXT if model['engine'] == 'kokoro' else DEFAULT_TEXT)
        ...
        for key, value in values.items():
            ...
            if key == 'consent':
                ...
                if key == 'voice':
                    display = tk.StringVar(value=KOKORO_VOICES[variable.get()])
                    choice = ttk.Combobox(row, textvariable=display, values=list(KOKORO_VOICES.values()), state='readonly', width=25)
                    choice.pack(side='right')
                    choice.bind('<<ComboboxSelected>>', lambda _, v=variable, d=display: v.set(next(k for k,label in KOKORO_VOICES.items() if label==d.get())))
                    variable.trace_add('write', lambda *_, v=variable, d=display: d.set(KOKORO_VOICES[v.get()]))
                elif key == 'language':
    ...
    def generate(self):
        ...
        try:
            ...
            self.saved['last_model_id'] = model['id']
