...
def main():
    ...
    for path in sorted(args.folder.glob('*.wav')):
        ...
        segments, info = model.transcribe(str(path), language='en', task='transcribe',
        ...
        report = dict(asr_model=str(args.model), model_sha256=fingerprint,
            ...
            settings=dict(language='en', task='transcribe', compute_type='int8', device='cpu',
                beam_size=5, temperature=0, condition_on_previous_text=False,
                vad_filter=False, initial_prompt=None),
            limitation='ASR can normalize spelling, numbers and punctuation or repair mispronunciation. '
